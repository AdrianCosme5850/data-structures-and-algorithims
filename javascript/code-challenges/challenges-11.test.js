...
function transformToLis(obj){
  ...
  let arr3 = [];
  arr1.forEach((keys, idx) => {
    arr3.push(`<li>${keys}: ${arr2[idx]}</li>`);
  });
  return arr3;
}
...
const count = (target, input) => {
  let value = 0;
  input.forEach(y => y.forEach(x => x === target ? value += 1 : value += 0));
  return value;
};
...
const totalSum = (input) => {
  let value = 0;
  input.forEach(currArr => currArr.forEach(currInteger => value += currInteger) );
  return value;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let newArr = [];
  input.forEach(array => {
    array.forEach(value => value % 5 === 0 ? newArr.push(value): console.log('miss') );
  });
  let finalArr = newArr.map(value => Math.pow(2, value));
  return finalArr;
};
...
let findShortest = (data) => {
  let short = data.reduce((shortValue, currValue) => Number(shortValue.height) < Number (currValue.height) ? shortValue : currValue).name;
  return short;
};
