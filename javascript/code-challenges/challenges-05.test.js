...
const toLastNames = people => {
  let newArr = people.map(name => name.firstName + ' ' + name.lastName);
  return newArr;
};
...
const addValues = (arr) => {
  let total = arr.reduce((runningTotal, newVal) => runningTotal + newVal, 0);
  return total;
};
...
const addPurchases = (arr) => {
  let priceTotal = arr.reduce((runningTotal, newValue) => runningTotal + newValue.purchasePrice, 0);
  return priceTotal;
};
...
const countNumberOfElements = (arr) => {
  let totalElements = arr.reduce((runningTotal, newValue) => runningTotal + newValue - newValue + 1);
