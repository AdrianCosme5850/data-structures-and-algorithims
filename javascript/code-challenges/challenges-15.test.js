...
const screenForNames = (arr) => {
  let newArr = [];
  let regex =/[MD][rs]+[.][ ][A-Za-z]+/gm;
  arr.forEach(string => regex.test(string) ? newArr.push(string): console.log('miss'));
  return newArr;
};
...
let biggerThanLuke = (arr) => {
  let mass = arr.find(char => char.name = 'Luke Skywalker');
  return arr.filter (char => parseInt(char.mass) > parseInt(mass.mass)).reduce((str, currentChar, i, arr) => {
...
const detectTicTacToeWin = (board) => {
  ...
  if(helpCheck([board[0][2], board[1][2], board[2][0]])) return true;
