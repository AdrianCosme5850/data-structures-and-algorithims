...
const sortByChildren = (charArray) => {
  let newArr = charArray.sort((a,b) => {
    let house1 = a.children.length;
    let house2 = b.children.length;
...
const isCapitalized = (str) => {
  ...
  matches ? matches.forEach(match => newArr.push(match)): newArr = [];
  return newArr;
...
const citiesAtoJ = (arr) => {
  let newArr = [];
  let regex = /^[A-J]/;
  arr.forEach(city => regex.test(city) ? newArr.push(city): console.log('miss'));
  return newArr;
};
