...
const replaceZeros = (string) => {
  ...
  let fixedArr = [];
  newArr.forEach(letter => regex.test(letter) ? fixedArr.push('zero') : fixedArr.push(letter));
...
const validateEmail = (email) => {
  let regex = /^\w+[a-zA-Z0-9.]{1}\w+[@][a-zA-Z0-9]+[.][netorgcm]+$/gm;
...
const validatePhoneNumber = (phoneNumber) => {
  let regex = /^[0-9]{3}[  -]{1}[0-9]{3}[ -]{1}[0-9]{4}$|[(][0-9]{3}[)][0-9 -]+[0-9]$|^[0-9]{3}[  -]{1}[0-9]{3}[0-9]{4}$|^[0-9]{3}[0-9]{3}[ -]{1}[0-9]{4}$|^[0-9]{10}$/gm;
